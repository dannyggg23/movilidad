...
<?php include_once "Ingreso_Bienessmryinfo.php" ?>
<?php
...
$Ingreso_Bienes_summary = NULL; // Initialize page object first

class crIngreso_Bienes_summary extends crIngreso_Bienes {
	...
	// Page object name
	var $PageObjName = 'Ingreso_Bienes_summary';
	...
	function __construct() {
		...
		// Table object (Ingreso_Bienes)
		if (!isset($GLOBALS["Ingreso_Bienes"])) {
			$GLOBALS["Ingreso_Bienes"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Ingreso_Bienes"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Ingreso-Bienes', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fIngreso_Bienessummary";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Ingreso_Bienes\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Ingreso_Bienes',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fIngreso_Bienessummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fIngreso_Bienessummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fIngreso_Bienessummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->idingreso_bienes->SetVisibility();
		$this->numero_ingreso->SetVisibility();
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['idingreso_bienes'] = ewr_Conv($rs->fields('idingreso_bienes'), 3);
				$this->FirstRowData['numero_ingreso'] = ewr_Conv($rs->fields('numero_ingreso'), 200);
				...
				$this->FirstRowData['n_documento'] = ewr_Conv($rs->fields('n_documento'), 200);
				$this->FirstRowData['estado'] = ewr_Conv($rs->fields('estado'), 200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			$this->idingreso_bienes->setDbValue($rs->fields('idingreso_bienes'));
			$this->numero_ingreso->setDbValue($rs->fields('numero_ingreso'));
			...
			$this->Val[1] = $this->idingreso_bienes->CurrentValue;
			$this->Val[2] = $this->numero_ingreso->CurrentValue;
			...
			$this->Val[8] = $this->estado->CurrentValue;
		} else {
			$this->idingreso_bienes->setDbValue("");
			$this->numero_ingreso->setDbValue("");
			$this->nombre->setDbValue("");
			$this->fecha->setDbValue("");
			$this->ubicacion->setDbValue("");
			$this->detalle->setDbValue("");
			$this->total->setDbValue("");
			$this->n_documento->setDbValue("");
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Ingreso_Bienes_fecha') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Ingreso_Bienes_fecha"])) {
			$this->LoadSelectionFromSession('fecha');
		} elseif (@$_SESSION["sel_Ingreso_Bienes_fecha"] == EWR_INIT_VALUE) { // Select all
			$this->fecha->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// idingreso_bienes
			$this->idingreso_bienes->HrefValue = "";
			if ($this->Export == "") {
				$drillurl = $this->idingreso_bienes->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->idingreso_bienes, "ingreso_bienes_idingreso_bienes", $this->RowTotalType, -1)), $drillurl);
				$this->idingreso_bienes->LinkAttrs["title"] = ewr_JsEncode($GLOBALS["ReportLanguage"]->Phrase("ClickToDrillDown"));
				$this->idingreso_bienes->LinkAttrs["class"] = "ewDrillLink";
				$this->idingreso_bienes->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Ingreso_Bienes_idingreso_bienes', $GLOBALS["ReportLanguage"]->TablePhrase('r_detalle_ingreso_bienes', 'TblCaption'), $this->UseDrillDownPanel);
			}

			// numero_ingreso
			$this->numero_ingreso->HrefValue = "";
			...
			// idingreso_bienes
			$this->idingreso_bienes->ViewValue = $this->idingreso_bienes->CurrentValue;
			$this->idingreso_bienes->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// numero_ingreso
			$this->numero_ingreso->ViewValue = $this->numero_ingreso->CurrentValue;
			$this->numero_ingreso->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			if ($this->Export == "") {
				$drillurl = $this->idingreso_bienes->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->idingreso_bienes, "ingreso_bienes_idingreso_bienes", 0)), $drillurl);
				$this->idingreso_bienes->LinkAttrs["title"] = ewr_JsEncode($ReportLanguage->Phrase("ClickToDrillDown"));
				$this->idingreso_bienes->LinkAttrs["class"] = "ewDrillLink";
				$this->idingreso_bienes->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Ingreso_Bienes_idingreso_bienes', $GLOBALS["ReportLanguage"]->TablePhrase('r_detalle_ingreso_bienes', 'TblCaption'), $this->UseDrillDownPanel);
			}
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// idingreso_bienes
			$CurrentValue = $this->idingreso_bienes->CurrentValue;
			$ViewValue = &$this->idingreso_bienes->ViewValue;
			$ViewAttrs = &$this->idingreso_bienes->ViewAttrs;
			$CellAttrs = &$this->idingreso_bienes->CellAttrs;
			$HrefValue = &$this->idingreso_bienes->HrefValue;
			$LinkAttrs = &$this->idingreso_bienes->LinkAttrs;
			$this->Cell_Rendered($this->idingreso_bienes, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// numero_ingreso
			$CurrentValue = $this->numero_ingreso->CurrentValue;
			$ViewValue = &$this->numero_ingreso->ViewValue;
			$ViewAttrs = &$this->numero_ingreso->ViewAttrs;
			$CellAttrs = &$this->numero_ingreso->CellAttrs;
			$HrefValue = &$this->numero_ingreso->HrefValue;
			$LinkAttrs = &$this->numero_ingreso->LinkAttrs;
			$this->Cell_Rendered($this->numero_ingreso, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->idingreso_bienes->Visible) $this->DtlColumnCount += 1;
		if ($this->numero_ingreso->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'Ingreso_Bienes_fecha')
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Ingreso_Bienes_fecha'] = ($this->fecha->SelectionList == "") ? EWR_INIT_VALUE : $this->fecha->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Ingreso_Bienes_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Ingreso_Bienes_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Ingreso_Bienes_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Ingreso_Bienes_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Ingreso_Bienes_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Ingreso_Bienes_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Ingreso_Bienes_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Ingreso_Bienes_' . $parm] = $sv;
		$_SESSION['so_Ingreso_Bienes_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Ingreso_Bienes_' . $parm] = $sv1;
		$_SESSION['so_Ingreso_Bienes_' . $parm] = $so1;
		$_SESSION['sc_Ingreso_Bienes_' . $parm] = $sc;
		$_SESSION['sv2_Ingreso_Bienes_' . $parm] = $sv2;
		$_SESSION['so2_Ingreso_Bienes_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Ingreso_Bienes_$parm"] = "";
		$_SESSION["rf_Ingreso_Bienes_$parm"] = "";
		$_SESSION["rt_Ingreso_Bienes_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_Ingreso_Bienes_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Ingreso_Bienes_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Ingreso_Bienes_$parm"];
	}
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_fecha", $filter)) {
			...
			$_SESSION["sel_Ingreso_Bienes_fecha"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "fecha");
			$this->fecha->SelectionList = "";
			$_SESSION["sel_Ingreso_Bienes_fecha"] = "";
		}
		return TRUE;
	}
	...
	function GetSort($options = array()) {
		...
		if ($bResetSort) {
			...
			$this->idingreso_bienes->setSort("");
			$this->numero_ingreso->setSort("");
			$this->nombre->setSort("");
			$this->fecha->setSort("");
...
// Create page object
if (!isset($Ingreso_Bienes_summary)) $Ingreso_Bienes_summary = new crIngreso_Bienes_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Ingreso_Bienes_summary;
...
// Create page object
var Ingreso_Bienes_summary = new ewr_Page("Ingreso_Bienes_summary");

// Page properties
Ingreso_Bienes_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Ingreso_Bienes_summary.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fIngreso_Bienessummary = new ewr_Form("fIngreso_Bienessummary");

// Validate method
fIngreso_Bienessummary.Validate = function() {
...
fIngreso_Bienessummary.Form_CustomValidate = 
...
fIngreso_Bienessummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fIngreso_Bienessummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fIngreso_Bienessummary" id="fIngreso_Bienessummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fIngreso_Bienessummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="Ingreso_Bienes" data-field="x_fecha" id="sv_fecha" name="sv_fecha" placeholder="<?php echo $Page->fecha->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->fecha->SearchValue) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":0}'<?php echo $Page->fecha->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->fecha->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Ingreso_Bienes" data-field="x_fecha" id="sv2_fecha" name="sv2_fecha" placeholder="<?php echo $Page->fecha->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->fecha->SearchValue2) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":0}'<?php echo $Page->fecha->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fIngreso_Bienessummary.Init();
fIngreso_Bienessummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Ingreso_Bienessmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_Ingreso_Bienes" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->idingreso_bienes->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="idingreso_bienes"><div class="Ingreso_Bienes_idingreso_bienes"><span class="ewTableHeaderCaption"><?php echo $Page->idingreso_bienes->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="idingreso_bienes">
<?php if ($Page->SortUrl($Page->idingreso_bienes) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_idingreso_bienes">
			<span class="ewTableHeaderCaption"><?php echo $Page->idingreso_bienes->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_idingreso_bienes" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->idingreso_bienes) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->idingreso_bienes->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->idingreso_bienes->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->idingreso_bienes->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->numero_ingreso->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="numero_ingreso"><div class="Ingreso_Bienes_numero_ingreso"><span class="ewTableHeaderCaption"><?php echo $Page->numero_ingreso->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="numero_ingreso">
<?php if ($Page->SortUrl($Page->numero_ingreso) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_numero_ingreso">
			<span class="ewTableHeaderCaption"><?php echo $Page->numero_ingreso->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_numero_ingreso" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->numero_ingreso) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->numero_ingreso->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->numero_ingreso->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->numero_ingreso->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombre"><div class="Ingreso_Bienes_nombre"><span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombre">
<?php if ($Page->SortUrl($Page->nombre) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fecha"><div class="Ingreso_Bienes_fecha"><span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fecha">
<?php if ($Page->SortUrl($Page->fecha) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_fecha">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'Ingreso_Bienes_fecha', range: true, from: '<?php echo $Page->fecha->RangeFrom; ?>', to: '<?php echo $Page->fecha->RangeTo; ?>', url: 'Ingreso_Bienessmry.php' });" id="x_fecha<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_fecha" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fecha) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ubicacion"><div class="Ingreso_Bienes_ubicacion"><span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ubicacion">
<?php if ($Page->SortUrl($Page->ubicacion) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_ubicacion">
			<span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_ubicacion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ubicacion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="detalle"><div class="Ingreso_Bienes_detalle"><span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="detalle">
<?php if ($Page->SortUrl($Page->detalle) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_detalle">
			<span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_detalle" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->detalle) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="total"><div class="Ingreso_Bienes_total"><span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="total">
<?php if ($Page->SortUrl($Page->total) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_total">
			<span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_total" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->total) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="estado"><div class="Ingreso_Bienes_estado"><span class="ewTableHeaderCaption"><?php echo $Page->estado->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="estado">
<?php if ($Page->SortUrl($Page->estado) == "") { ?>
		<div class="ewTableHeaderBtn Ingreso_Bienes_estado">
			<span class="ewTableHeaderCaption"><?php echo $Page->estado->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Ingreso_Bienes_estado" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->estado) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->estado->FldCaption() ?></span>
...
<?php if ($Page->idingreso_bienes->Visible) { ?>
		<td data-field="idingreso_bienes"<?php echo $Page->idingreso_bienes->CellAttributes() ?>>
<span<?php echo $Page->idingreso_bienes->ViewAttributes() ?>>
<?php if ($Page->idingreso_bienes->HrefValue <> "" || @$Page->idingreso_bienes->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->idingreso_bienes->ListViewValue() <> "" && $Page->idingreso_bienes->ListViewValue() <> "&nbsp;") { ?>
<a<?php echo $Page->idingreso_bienes->LinkAttributes() ?>><?php echo $Page->idingreso_bienes->ListViewValue() ?></a>
<?php } else { echo "&nbsp;"; } ?>
<?php } else { ?>
<?php if ($Page->idingreso_bienes->ListViewValue() <> "" && $Page->idingreso_bienes->ListViewValue() <> "&nbsp;") { ?>
<?php echo $Page->idingreso_bienes->ListViewValue() ?>
<?php } else { echo "&nbsp;"; } ?>
<?php } ?>
</span></td>
<?php } ?>
<?php if ($Page->numero_ingreso->Visible) { ?>
		<td data-field="numero_ingreso"<?php echo $Page->numero_ingreso->CellAttributes() ?>>
<span<?php echo $Page->numero_ingreso->ViewAttributes() ?>><?php echo $Page->numero_ingreso->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->nombre->Visible) { ?>
...
<?php } ?>
<?php if ($Page->idingreso_bienes->Visible) { ?>
		<td data-field="idingreso_bienes"<?php echo $Page->idingreso_bienes->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->numero_ingreso->Visible) { ?>
		<td data-field="numero_ingreso"<?php echo $Page->numero_ingreso->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->nombre->Visible) { ?>
