...
<?php include_once "r_detalle_ingreso_especiesrptinfo.php" ?>
<?php
...
$r_detalle_ingreso_especies_rpt = NULL; // Initialize page object first

class crr_detalle_ingreso_especies_rpt extends crr_detalle_ingreso_especies {
	...
	// Page object name
	var $PageObjName = 'r_detalle_ingreso_especies_rpt';
	...
	function __construct() {
		...
		// Table object (r_detalle_ingreso_especies)
		if (!isset($GLOBALS["r_detalle_ingreso_especies"])) {
			$GLOBALS["r_detalle_ingreso_especies"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_detalle_ingreso_especies"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_detalle_ingreso_especies', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_detalle_ingreso_especiesrpt";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_detalle_ingreso_especies\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_detalle_ingreso_especies',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_detalle_ingreso_especiesrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_detalle_ingreso_especiesrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_detalle_ingreso_especiesrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Main() {
		...
		$this->iddetalle_ingreso_especies->SetVisibility();
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['iddetalle_ingreso_especies'] = ewr_Conv($rs->fields('iddetalle_ingreso_especies'), 3);
				$this->FirstRowData['nombre'] = ewr_Conv($rs->fields('nombre'), 200);
				$this->FirstRowData['ingreso_especies_idingreso_especies'] = ewr_Conv($rs->fields('ingreso_especies_idingreso_especies'), 3);
		...
		if (!$rs->EOF) {
			$this->iddetalle_ingreso_especies->setDbValue($rs->fields('iddetalle_ingreso_especies'));
			$this->nombre->setDbValue($rs->fields('nombre'));
			$this->ingreso_especies_idingreso_especies->setDbValue($rs->fields('ingreso_especies_idingreso_especies'));
			$this->cantidad->setDbValue($rs->fields('cantidad'));
			$this->desde->setDbValue($rs->fields('desde'));
			$this->hasta->setDbValue($rs->fields('hasta'));
			$this->Val[1] = $this->iddetalle_ingreso_especies->CurrentValue;
			$this->Val[2] = $this->nombre->CurrentValue;
			$this->Val[3] = $this->cantidad->CurrentValue;
			$this->Val[4] = $this->desde->CurrentValue;
			$this->Val[5] = $this->hasta->CurrentValue;
		} else {
			$this->iddetalle_ingreso_especies->setDbValue("");
			$this->nombre->setDbValue("");
			$this->ingreso_especies_idingreso_especies->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// iddetalle_ingreso_especies
			$this->iddetalle_ingreso_especies->HrefValue = "";
			...
			// iddetalle_ingreso_especies
			$this->iddetalle_ingreso_especies->ViewValue = $this->iddetalle_ingreso_especies->CurrentValue;
			$this->iddetalle_ingreso_especies->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// iddetalle_ingreso_especies
			$CurrentValue = $this->iddetalle_ingreso_especies->CurrentValue;
			$ViewValue = &$this->iddetalle_ingreso_especies->ViewValue;
			$ViewAttrs = &$this->iddetalle_ingreso_especies->ViewAttrs;
			$CellAttrs = &$this->iddetalle_ingreso_especies->CellAttrs;
			$HrefValue = &$this->iddetalle_ingreso_especies->HrefValue;
			$LinkAttrs = &$this->iddetalle_ingreso_especies->LinkAttrs;
			$this->Cell_Rendered($this->iddetalle_ingreso_especies, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->iddetalle_ingreso_especies->Visible) $this->DtlColumnCount += 1;
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$sql = @$post["ingreso_especies_idingreso_especies"];
			...
			$sql = str_replace("$", "-", $sql);
			$sql = ewr_Decrypt($sql);
			$sql = str_replace("@ingreso_especies_idingreso_especies", "`ingreso_especies_idingreso_especies`", $sql);
			if ($sql <> "") {
				if ($filter <> "") $filter .= " AND ";
				$filter .= $sql;
				if ($sql <> "1=1")
					$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->ingreso_especies_idingreso_especies->FldCaption() . "</span><span class=\"ewFilterValue\">$sql</span></div>";
			}

			// Save to session
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = $mastertable;
			$_SESSION['do_r_detalle_ingreso_especies'] = $opt;
			$_SESSION['df_r_detalle_ingreso_especies'] = $filter;
			$_SESSION['dl_r_detalle_ingreso_especies'] = $sFilterList;
		} elseif (@$_GET["cmd"] == "resetdrilldown") { // Clear drill down
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = "";
			$_SESSION['do_r_detalle_ingreso_especies'] = "";
			$_SESSION['df_r_detalle_ingreso_especies'] = "";
			$_SESSION['dl_r_detalle_ingreso_especies'] = "";
		} else { // Restore from Session
			$opt = @$_SESSION['do_r_detalle_ingreso_especies'];
			$filter = @$_SESSION['df_r_detalle_ingreso_especies'];
			$sFilterList = @$_SESSION['dl_r_detalle_ingreso_especies'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
	...
	function GetSort($options = array()) {
		...
		if ($bResetSort) {
			...
			$this->iddetalle_ingreso_especies->setSort("");
			$this->nombre->setSort("");
...
// Create page object
if (!isset($r_detalle_ingreso_especies_rpt)) $r_detalle_ingreso_especies_rpt = new crr_detalle_ingreso_especies_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_detalle_ingreso_especies_rpt;
...
// Create page object
var r_detalle_ingreso_especies_rpt = new ewr_Page("r_detalle_ingreso_especies_rpt");

// Page properties
r_detalle_ingreso_especies_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = r_detalle_ingreso_especies_rpt.PageID;
</script>
<?php } ?>
...
<?php include "r_detalle_ingreso_especiesrptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_r_detalle_ingreso_especies" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->iddetalle_ingreso_especies->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="iddetalle_ingreso_especies"><div class="r_detalle_ingreso_especies_iddetalle_ingreso_especies"><span class="ewTableHeaderCaption"><?php echo $Page->iddetalle_ingreso_especies->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="iddetalle_ingreso_especies">
<?php if ($Page->SortUrl($Page->iddetalle_ingreso_especies) == "") { ?>
		<div class="ewTableHeaderBtn r_detalle_ingreso_especies_iddetalle_ingreso_especies">
			<span class="ewTableHeaderCaption"><?php echo $Page->iddetalle_ingreso_especies->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_detalle_ingreso_especies_iddetalle_ingreso_especies" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->iddetalle_ingreso_especies) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->iddetalle_ingreso_especies->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->iddetalle_ingreso_especies->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->iddetalle_ingreso_especies->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombre"><div class="r_detalle_ingreso_especies_nombre"><span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombre">
<?php if ($Page->SortUrl($Page->nombre) == "") { ?>
		<div class="ewTableHeaderBtn r_detalle_ingreso_especies_nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_detalle_ingreso_especies_nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="cantidad"><div class="r_detalle_ingreso_especies_cantidad"><span class="ewTableHeaderCaption"><?php echo $Page->cantidad->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="cantidad">
<?php if ($Page->SortUrl($Page->cantidad) == "") { ?>
		<div class="ewTableHeaderBtn r_detalle_ingreso_especies_cantidad">
			<span class="ewTableHeaderCaption"><?php echo $Page->cantidad->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_detalle_ingreso_especies_cantidad" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->cantidad) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->cantidad->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="desde"><div class="r_detalle_ingreso_especies_desde"><span class="ewTableHeaderCaption"><?php echo $Page->desde->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="desde">
<?php if ($Page->SortUrl($Page->desde) == "") { ?>
		<div class="ewTableHeaderBtn r_detalle_ingreso_especies_desde">
			<span class="ewTableHeaderCaption"><?php echo $Page->desde->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_detalle_ingreso_especies_desde" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->desde) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->desde->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="hasta"><div class="r_detalle_ingreso_especies_hasta"><span class="ewTableHeaderCaption"><?php echo $Page->hasta->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="hasta">
<?php if ($Page->SortUrl($Page->hasta) == "") { ?>
		<div class="ewTableHeaderBtn r_detalle_ingreso_especies_hasta">
			<span class="ewTableHeaderCaption"><?php echo $Page->hasta->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_detalle_ingreso_especies_hasta" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->hasta) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->hasta->FldCaption() ?></span>
...
<?php if ($Page->iddetalle_ingreso_especies->Visible) { ?>
		<td data-field="iddetalle_ingreso_especies"<?php echo $Page->iddetalle_ingreso_especies->CellAttributes() ?>>
<span<?php echo $Page->iddetalle_ingreso_especies->ViewAttributes() ?>><?php echo $Page->iddetalle_ingreso_especies->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->nombre->Visible) { ?>
