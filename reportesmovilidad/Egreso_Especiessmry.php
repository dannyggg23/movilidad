...
<?php include_once "Egreso_Especiessmryinfo.php" ?>
<?php
...
$Egreso_Especies_summary = NULL; // Initialize page object first

class crEgreso_Especies_summary extends crEgreso_Especies {
	...
	// Page object name
	var $PageObjName = 'Egreso_Especies_summary';
	...
	function __construct() {
		...
		// Table object (Egreso_Especies)
		if (!isset($GLOBALS["Egreso_Especies"])) {
			$GLOBALS["Egreso_Especies"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Egreso_Especies"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Egreso-Especies', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fEgreso_Especiessummary";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewrExportLink ewEmail\" title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Egreso_Especies\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Egreso_Especies',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fEgreso_Especiessummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fEgreso_Especiessummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fEgreso_Especiessummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->idegreso_especies->SetVisibility();
		$this->numero_documento->SetVisibility();
		...
		$this->Col = array(array(FALSE, FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE), array(FALSE,FALSE));
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['idegreso_especies'] = ewr_Conv($rs->fields('idegreso_especies'), 3);
				$this->FirstRowData['numero_documento'] = ewr_Conv($rs->fields('numero_documento'), 200);
				$this->FirstRowData['fecha'] = ewr_Conv($rs->fields('fecha'), 133);
				$this->FirstRowData['nombre'] = ewr_Conv($rs->fields('nombre'), 200);
				$this->FirstRowData['persona'] = ewr_Conv($rs->fields('persona'), 200);
		...
		if (!$rs->EOF) {
			$this->idegreso_especies->setDbValue($rs->fields('idegreso_especies'));
			$this->numero_documento->setDbValue($rs->fields('numero_documento'));
			...
			$this->Val[1] = $this->idegreso_especies->CurrentValue;
			$this->Val[2] = $this->numero_documento->CurrentValue;
			$this->Val[3] = $this->fecha->CurrentValue;
			$this->Val[4] = $this->nombre->CurrentValue;
			$this->Val[5] = $this->persona->CurrentValue;
			$this->Val[6] = $this->ubicacion->CurrentValue;
			$this->Val[7] = $this->detalle->CurrentValue;
			$this->Val[8] = $this->total->CurrentValue;
			$this->Val[9] = $this->condicion->CurrentValue;
		} else {
			$this->idegreso_especies->setDbValue("");
			$this->numero_documento->setDbValue("");
			$this->fecha->setDbValue("");
			$this->nombre->setDbValue("");
			$this->persona->setDbValue("");
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Egreso_Especies_fecha') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Egreso_Especies_fecha"])) {
			$this->LoadSelectionFromSession('fecha');
		} elseif (@$_SESSION["sel_Egreso_Especies_fecha"] == EWR_INIT_VALUE) { // Select all
			$this->fecha->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// idegreso_especies
			$this->idegreso_especies->HrefValue = "";
			if ($this->Export == "") {
				$drillurl = $this->idegreso_especies->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->idegreso_especies, "egreso_especies_idegreso_especies", $this->RowTotalType, -1)), $drillurl);
				$this->idegreso_especies->LinkAttrs["title"] = ewr_JsEncode($GLOBALS["ReportLanguage"]->Phrase("ClickToDrillDown"));
				$this->idegreso_especies->LinkAttrs["class"] = "ewDrillLink";
				$this->idegreso_especies->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Egreso_Especies_idegreso_especies', $GLOBALS["ReportLanguage"]->TablePhrase('r_detalle_egreso_especies', 'TblCaption'), $this->UseDrillDownPanel);
			}

			// numero_documento
			$this->numero_documento->HrefValue = "";
			...
			// idegreso_especies
			$this->idegreso_especies->ViewValue = $this->idegreso_especies->CurrentValue;
			$this->idegreso_especies->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// numero_documento
			$this->numero_documento->ViewValue = $this->numero_documento->CurrentValue;
			$this->numero_documento->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			$this->persona->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			if ($this->Export == "") {
				$drillurl = $this->idegreso_especies->DrillDownUrl;
				$drillurl = str_replace("=f0", "=" . ewr_Encrypt($this->GetDrillDownSQL($this->idegreso_especies, "egreso_especies_idegreso_especies", 0)), $drillurl);
				$this->idegreso_especies->LinkAttrs["title"] = ewr_JsEncode($ReportLanguage->Phrase("ClickToDrillDown"));
				$this->idegreso_especies->LinkAttrs["class"] = "ewDrillLink";
				$this->idegreso_especies->LinkAttrs["onclick"] = ewr_DrillDownJs($drillurl, 'Egreso_Especies_idegreso_especies', $GLOBALS["ReportLanguage"]->TablePhrase('r_detalle_egreso_especies', 'TblCaption'), $this->UseDrillDownPanel);
			}
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// idegreso_especies
			$CurrentValue = $this->idegreso_especies->CurrentValue;
			$ViewValue = &$this->idegreso_especies->ViewValue;
			$ViewAttrs = &$this->idegreso_especies->ViewAttrs;
			$CellAttrs = &$this->idegreso_especies->CellAttrs;
			$HrefValue = &$this->idegreso_especies->HrefValue;
			$LinkAttrs = &$this->idegreso_especies->LinkAttrs;
			$this->Cell_Rendered($this->idegreso_especies, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// numero_documento
			$CurrentValue = $this->numero_documento->CurrentValue;
			$ViewValue = &$this->numero_documento->ViewValue;
			$ViewAttrs = &$this->numero_documento->ViewAttrs;
			$CellAttrs = &$this->numero_documento->CellAttrs;
			$HrefValue = &$this->numero_documento->HrefValue;
			$LinkAttrs = &$this->numero_documento->LinkAttrs;
			$this->Cell_Rendered($this->numero_documento, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
			...
			// persona
			$CurrentValue = $this->persona->CurrentValue;
			$ViewValue = &$this->persona->ViewValue;
			$ViewAttrs = &$this->persona->ViewAttrs;
			$CellAttrs = &$this->persona->CellAttrs;
			$HrefValue = &$this->persona->HrefValue;
			$LinkAttrs = &$this->persona->LinkAttrs;
			$this->Cell_Rendered($this->persona, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->idegreso_especies->Visible) $this->DtlColumnCount += 1;
		if ($this->numero_documento->Visible) $this->DtlColumnCount += 1;
		if ($this->fecha->Visible) $this->DtlColumnCount += 1;
		if ($this->nombre->Visible) $this->DtlColumnCount += 1;
		if ($this->persona->Visible) $this->DtlColumnCount += 1;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'Egreso_Especies_fecha')
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_Egreso_Especies_fecha'] = ($this->fecha->SelectionList == "") ? EWR_INIT_VALUE : $this->fecha->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_Egreso_Especies_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Egreso_Especies_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_Egreso_Especies_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_Egreso_Especies_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_Egreso_Especies_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_Egreso_Especies_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_Egreso_Especies_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_Egreso_Especies_' . $parm] = $sv;
		$_SESSION['so_Egreso_Especies_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_Egreso_Especies_' . $parm] = $sv1;
		$_SESSION['so_Egreso_Especies_' . $parm] = $so1;
		$_SESSION['sc_Egreso_Especies_' . $parm] = $sc;
		$_SESSION['sv2_Egreso_Especies_' . $parm] = $sv2;
		$_SESSION['so2_Egreso_Especies_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Egreso_Especies_$parm"] = "";
		$_SESSION["rf_Egreso_Especies_$parm"] = "";
		$_SESSION["rt_Egreso_Especies_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_Egreso_Especies_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Egreso_Especies_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Egreso_Especies_$parm"];
	}
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_fecha", $filter)) {
			...
			$_SESSION["sel_Egreso_Especies_fecha"] = $sWrk;
		...
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "fecha");
			$this->fecha->SelectionList = "";
			$_SESSION["sel_Egreso_Especies_fecha"] = "";
		}
		return TRUE;
	}
	...
	function GetSort($options = array()) {
		...
		if ($bResetSort) {
			...
			$this->idegreso_especies->setSort("");
			$this->numero_documento->setSort("");
			$this->fecha->setSort("");
			$this->nombre->setSort("");
...
// Create page object
if (!isset($Egreso_Especies_summary)) $Egreso_Especies_summary = new crEgreso_Especies_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Egreso_Especies_summary;
...
// Create page object
var Egreso_Especies_summary = new ewr_Page("Egreso_Especies_summary");

// Page properties
Egreso_Especies_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Egreso_Especies_summary.PageID;
</script>
<?php } ?>
...
// Form object
var CurrentForm = fEgreso_Especiessummary = new ewr_Form("fEgreso_Especiessummary");

// Validate method
fEgreso_Especiessummary.Validate = function() {
...
fEgreso_Especiessummary.Form_CustomValidate = 
...
fEgreso_Especiessummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fEgreso_Especiessummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown && !$grDashboardReport) { ?>
<!-- Search form (begin) -->
<form name="fEgreso_Especiessummary" id="fEgreso_Especiessummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fEgreso_Especiessummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="Egreso_Especies" data-field="x_fecha" id="sv_fecha" name="sv_fecha" placeholder="<?php echo $Page->fecha->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->fecha->SearchValue) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":0}'<?php echo $Page->fecha->EditAttributes() ?>>
</span>
...
<?php ewr_PrependClass($Page->fecha->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="Egreso_Especies" data-field="x_fecha" id="sv2_fecha" name="sv2_fecha" placeholder="<?php echo $Page->fecha->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->fecha->SearchValue2) ?>" data-calendar='true' data-options='{"ignoreReadonly":true,"useCurrent":false,"format":0}'<?php echo $Page->fecha->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fEgreso_Especiessummary.Init();
fEgreso_Especiessummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Egreso_Especiessmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_Egreso_Especies" class="<?php if (ewr_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php } ?>
<table class="<?php echo $Page->ReportTableClass ?>">
<thead>
	<!-- Table header -->
	<tr class="ewTableHeader">
<?php if ($Page->idegreso_especies->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="idegreso_especies"><div class="Egreso_Especies_idegreso_especies"><span class="ewTableHeaderCaption"><?php echo $Page->idegreso_especies->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="idegreso_especies">
<?php if ($Page->SortUrl($Page->idegreso_especies) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_idegreso_especies">
			<span class="ewTableHeaderCaption"><?php echo $Page->idegreso_especies->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_idegreso_especies" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->idegreso_especies) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->idegreso_especies->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->idegreso_especies->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->idegreso_especies->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->numero_documento->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="numero_documento"><div class="Egreso_Especies_numero_documento"><span class="ewTableHeaderCaption"><?php echo $Page->numero_documento->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="numero_documento">
<?php if ($Page->SortUrl($Page->numero_documento) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_numero_documento">
			<span class="ewTableHeaderCaption"><?php echo $Page->numero_documento->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_numero_documento" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->numero_documento) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->numero_documento->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->numero_documento->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->numero_documento->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="fecha"><div class="Egreso_Especies_fecha"><span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="fecha">
<?php if ($Page->SortUrl($Page->fecha) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_fecha">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
	<?php if (!$grDashboardReport) { ?>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, { name: 'Egreso_Especies_fecha', range: true, from: '<?php echo $Page->fecha->RangeFrom; ?>', to: '<?php echo $Page->fecha->RangeTo; ?>', url: 'Egreso_Especiessmry.php' });" id="x_fecha<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
	<?php } ?>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_fecha" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->fecha) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->fecha->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="nombre"><div class="Egreso_Especies_nombre"><span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="nombre">
<?php if ($Page->SortUrl($Page->nombre) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_nombre">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_nombre" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->nombre) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->nombre->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->persona->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="persona"><div class="Egreso_Especies_persona"><span class="ewTableHeaderCaption"><?php echo $Page->persona->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="persona">
<?php if ($Page->SortUrl($Page->persona) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_persona">
			<span class="ewTableHeaderCaption"><?php echo $Page->persona->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_persona" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->persona) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->persona->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->persona->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->persona->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ubicacion"><div class="Egreso_Especies_ubicacion"><span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ubicacion">
<?php if ($Page->SortUrl($Page->ubicacion) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_ubicacion">
			<span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_ubicacion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ubicacion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ubicacion->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="detalle"><div class="Egreso_Especies_detalle"><span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="detalle">
<?php if ($Page->SortUrl($Page->detalle) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_detalle">
			<span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_detalle" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->detalle) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->detalle->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="total"><div class="Egreso_Especies_total"><span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="total">
<?php if ($Page->SortUrl($Page->total) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_total">
			<span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_total" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->total) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->total->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="condicion"><div class="Egreso_Especies_condicion"><span class="ewTableHeaderCaption"><?php echo $Page->condicion->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="condicion">
<?php if ($Page->SortUrl($Page->condicion) == "") { ?>
		<div class="ewTableHeaderBtn Egreso_Especies_condicion">
			<span class="ewTableHeaderCaption"><?php echo $Page->condicion->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Egreso_Especies_condicion" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->condicion) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->condicion->FldCaption() ?></span>
...
<?php if ($Page->idegreso_especies->Visible) { ?>
		<td data-field="idegreso_especies"<?php echo $Page->idegreso_especies->CellAttributes() ?>>
<span<?php echo $Page->idegreso_especies->ViewAttributes() ?>>
<?php if ($Page->idegreso_especies->HrefValue <> "" || @$Page->idegreso_especies->LinkAttrs["onclick"] <> "") { ?>
<?php if ($Page->idegreso_especies->ListViewValue() <> "" && $Page->idegreso_especies->ListViewValue() <> "&nbsp;") { ?>
<a<?php echo $Page->idegreso_especies->LinkAttributes() ?>><?php echo $Page->idegreso_especies->ListViewValue() ?></a>
<?php } else { echo "&nbsp;"; } ?>
<?php } else { ?>
<?php if ($Page->idegreso_especies->ListViewValue() <> "" && $Page->idegreso_especies->ListViewValue() <> "&nbsp;") { ?>
<?php echo $Page->idegreso_especies->ListViewValue() ?>
<?php } else { echo "&nbsp;"; } ?>
<?php } ?>
</span></td>
<?php } ?>
<?php if ($Page->numero_documento->Visible) { ?>
		<td data-field="numero_documento"<?php echo $Page->numero_documento->CellAttributes() ?>>
<span<?php echo $Page->numero_documento->ViewAttributes() ?>><?php echo $Page->numero_documento->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->fecha->Visible) { ?>
...
<?php } ?>
<?php if ($Page->persona->Visible) { ?>
		<td data-field="persona"<?php echo $Page->persona->CellAttributes() ?>>
<span<?php echo $Page->persona->ViewAttributes() ?>><?php echo $Page->persona->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ubicacion->Visible) { ?>
...
<?php } ?>
<?php if ($Page->idegreso_especies->Visible) { ?>
		<td data-field="idegreso_especies"<?php echo $Page->idegreso_especies->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->numero_documento->Visible) { ?>
		<td data-field="numero_documento"<?php echo $Page->numero_documento->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->fecha->Visible) { ?>
...
<?php } ?>
<?php if ($Page->persona->Visible) { ?>
		<td data-field="persona"<?php echo $Page->persona->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->ubicacion->Visible) { ?>
